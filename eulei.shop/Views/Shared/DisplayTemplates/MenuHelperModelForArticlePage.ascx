...
<%--<ul id="menu">
    <% foreach (var node in Model.Nodes)
       { %>
    <%if (!node.Area.Equals("Article")&&!node.IsRootNode)
      {
          continue;
      }
    %>
...
<ul id="menu">
    ...
    <li class="node">
        ...
        <% if (node.Children.Any())
           { %>
        <div class="nodeBox" style="position: absolute; left:<%=node.Children.Count>(Model.Nodes.Count-_i-1)?((Model.Nodes.Count-_i-2)-node.Children.Count)*90:0%>px; top: 30px; width: <%=node.Children.Count*110 %>px;">
            <div class="nodeBox-title"  style=" background-position: <%=node.Children.Count>(Model.Nodes.Count-_i-1)?-((Model.Nodes.Count-_i-2)-node.Children.Count)*90+40:40%>px bottom;">
            </div>
